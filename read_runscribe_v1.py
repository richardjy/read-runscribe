...
try:   #generic error trapping 
   ...
   # import datetime
    ...
    gap_step_rate = 0
    gap_stride_pace = 20  # arbitary. Larger value might be more realistic
    gap_stride_length = 0
    ...
    while True:
        ...
        for shoe in range(0, no_feet):
            ...
            active_miles = iactive_dist[0]/5280
            active_pace = datetime.min + timedelta(seconds=(iactive[0]/(1000*active_miles)))
            pause_miles = ipause_dist[0]/5280
            total_miles = active_miles + pause_miles
            distance_ratio = total_miles/active_miles #can be used to enter calibration value for run that includes pauses
            ...
            print(foot+ ' -' + '\tActive: {0:.2f}'.format(active_miles) + ' mi ' + showHMS(active_time) \
                  + ' ' + showMS(active_pace) + ' min/mi  Pause: ' + showHMS(pause_time) \
                  + '  Total: {0:.2f}'.format(total_miles) + ' mi ' + showHMS(total_time) \
                  + ' DistRatio: {0:.3f}'.format(distance_ratio))

            # append summary data to general logfile
            log_file.write(foot + ', ' + showHMS(active_time) + ', {0:.2f}'.format(active_miles) + ', ' \
                  + showMS(active_pace) + ', ' +  showHMS(pause_time) + ', {0:.2f}'.format(total_miles) + ', ' \
            ...
            for pt in range(1, no_pts):
                ...
                csv_file.write(pt_tod.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3] + ", " #show millisecs not microsecs \  
                               ...
                               + '{number:.{digits}f}, '.format(number=(active_dist[pt]/5280), digits=4) \
                               + '{number:.{digits}f}, '.format(number=(active[pt]/1000), digits=1) \
                               + '{number:.{digits}f}, '.format(number=(pause[pt]/1000), digits=1) \
